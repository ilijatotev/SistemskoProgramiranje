...
public class RequestHandler
{
    ...
    public void HandleRequest(object state)
    {
        ...
        {
            logger.Log($"[INFO][Nit {threadId}] Fajl {requestedFileName} pronadjen u kesu. Kesirana verzija.");
        }
        else
        {
            try
            {
                ...
                {
                    string errorMessage = $"Fajl '{requestedFileName}' nije pronadjen.";
                    response.StatusCode = (int)HttpStatusCode.NotFound;
                    byte[] buffer = System.Text.Encoding.UTF8.GetBytes(errorMessage);
                    response.ContentLength64 = buffer.Length;
                    response.OutputStream.Write(buffer, 0, buffer.Length);
                    logger.Log($"[GRESKA][Nit {threadId}] Fajl {requestedFileName} nije pronadjen.");
                ...
                logger.Log($"[INFO][Nit {threadId}] Fajl {requestedFileName} uspesno konvertovan, dodat u kes.");
            }
            catch (Exception ex)
            {
                string errorMessage = $"Doslo je do greske na serveru: {ex.Message}";
                response.StatusCode = (int)HttpStatusCode.InternalServerError;
                byte[] buffer = System.Text.Encoding.UTF8.GetBytes(errorMessage);
                response.ContentLength64 = buffer.Length;
                response.OutputStream.Write(buffer, 0, buffer.Length);
                logger.Log($"[GRESKA][Nit {threadId}] Greska pri obradi zahteva za {requestedFileName}: {ex.Message} | {DateTime.Now}");
                response.OutputStream.Close();
                return;
            }
        }
        ...
        logger.Log($"[LOG][Nit {threadId}] Zahtev za {requestedFileName} uspesno obradjen. | {DateTime.Now}");
    }
